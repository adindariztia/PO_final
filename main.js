...
function showPoSummary() {
    ...
    $.ajax({
        ...
        success: function(res) {
            ...
            display = document.getElementById('totalAmount')
            display.value = dataContract.total_price
            ...
            if (role === 'SCM'){
                ...
                $('#button-areas').append(`<button type="submit" class="btn btn-primary btn-custom mr-2" onclick="event.preventDefault(); requestersubmit('${dataContract.sap_contract_number}')" id="revision">Submit</button>`)
            }
        ...
        success: function(res){
            ...
            details.forEach(data =>{
                ...
                <div class="box-isi-comment">
                    <p class="font-weight-normal m-0 custom-name" id="userName">${data.username}</p>
                    <p class="font-weight-normal m-0 custom-comment" id="Comment">${data.comment}</p>
                    <p class="font-weight-normal m-0 custom-date" id="dateComment">${data.published}</p>
                </div>`)
...
function requestersubmit(sap_contract_number){
    var comment = $('#writeComment').val()
    $.ajax({
        method: 'POST',
        url: 'http://localhost:5000/submitToSCM',
...
function revision(){
    ...
    $.ajax({
        ...
        success: function(res){
            ...
            // console.log(tasklistOwner)
            // console.log(records)
            showrevision(tasklistOwner, records)
        },
        error: function(err){
            alert(err)
