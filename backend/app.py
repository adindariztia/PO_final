...
def submitDataPoToDatabase():
    ...
    dataContract.total_price = request_data['request_data']['totalAmount']
...
def scm_decision():
    if request.method == "POST":
            ...
            if userDB is not None:
                ...
                if result['data'] is None or len(result['data']) == 0:
                    ...
                    if req_decision == 'Approved':
                        ...
                        #gerakin dari scm ke requester lagi
                        requester_email = result['data'][0]['form_data']['pVRequester']
                        task_id = result['data'][0]['id']
                        submit_data = {
                        "data": {
                            "form_data": {
                                "pVRequester": requester_email,
                                "pVAction": req_decision
        ...
        if req_decision == 'Approved':
            ...
            return "flow submitted to manager"
        else:
            return "Flow submitted to Requester", 200
...
def ownerApproved():
    if request.method == "POST":
        ...
        recursive()
        submitApproval(req_username,contract_doc.id)
        return "PO Released. Congratulation!"
...
def getSummary():
    ...
    json_format = {
        ...
        "note" : headerDB.note,
        "total_price" : contract_doc.total_price,
